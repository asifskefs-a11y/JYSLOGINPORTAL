                    ...
                    coutBtn.onclick = () => {
                        window.openPasswordModal("Verify Check-Out", async () => {
                            try {
                                ...
                                // INJECT ONESIGNAL PUSH TRIGGER: Check-Out Success
                                window.OneSignalDeferred = window.OneSignalDeferred || [];
                                window.OneSignalDeferred.push(async function(OneSignal) {
                                    try {
                                        const pushId = OneSignal.User.PushSubscription.id;
                                        if (!pushId && Notification.permission === 'granted') await OneSignal.User.PushSubscription.optIn();

                                        // Fire high-fidelity confirmation
                                        new Notification("Check-Out Successful", {
                                            body: "Your check-out time has been logged.",
                                            icon: "jys_Icon.png"
                                        });
                                    } catch (err) { console.warn("OneSignal CheckOut Trigger Error:", err); }
                                });
                        ...
                        window.openPasswordModal("Verify Check-In", () => {
                            window.openSignatureModal("Staff Check-In Signature", async (sigData) => {
                                try {
                                    ...
                                    // INJECT ONESIGNAL PUSH TRIGGER: Check-In Success
                                    window.OneSignalDeferred = window.OneSignalDeferred || [];
                                    window.OneSignalDeferred.push(async function(OneSignal) {
                                        try {
                                            const pushId = OneSignal.User.PushSubscription.id;
                                        ...
                                        } catch (err) { console.warn("OneSignal CheckIn Trigger Error:", err); }
                                    });
